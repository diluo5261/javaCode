...
export default {
    ...
    //分页查询课程信息
    getCoursePage(current,limit){
        return request({
            url: `/eduservice/educourse/getCoursePage/${current}/${limit}`,
            method: 'get'
        })

    },
    //带条件的分页查询
    getCoursePageQuery(current,limit,courseQuery){
        return request({
            url: `/eduservice/educourse/getCoursePageVo/${current}/${limit}`,
            method: 'post',
            data: courseQuery //转化成json串进行传递
        })
    },
